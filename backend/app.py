...
try:
    ...
    def load_huggingface_model():
        ...
        try:
            ...
            try:
                ...
                with torch.no_grad():
                    dummy = torch.zeros((1, 20, 3, 112, 112)).to(device)
                    ml_model(dummy)
    ...
    def extract_frames_from_video(video_source, max_frames=20):
        try:
            if hasattr(video_source, "save"):
                temp_path = os.path.join(
                    TEMP_DIR,
                    f"upload_{int(time.time())}.mp4"
                )
                video_source.save(temp_path)
            else:
